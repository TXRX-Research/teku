...
import tech.pegasys.teku.phase1.integration.datastructures.AttesterSlashing
...
import tech.pegasys.teku.phase1.integration.datastructures.CompactCommittee
import tech.pegasys.teku.phase1.integration.datastructures.CustodyChunkChallenge
import tech.pegasys.teku.phase1.integration.datastructures.CustodyChunkChallengeRecord
import tech.pegasys.teku.phase1.integration.datastructures.CustodyChunkResponse
import tech.pegasys.teku.phase1.integration.datastructures.CustodyKeyReveal
import tech.pegasys.teku.phase1.integration.datastructures.Deposit
import tech.pegasys.teku.phase1.integration.datastructures.DepositData
import tech.pegasys.teku.phase1.integration.datastructures.DepositMessage
import tech.pegasys.teku.phase1.integration.datastructures.EarlyDerivedSecretReveal
import tech.pegasys.teku.phase1.integration.datastructures.Eth1Block
...
import tech.pegasys.teku.phase1.integration.datastructures.LightAggregateAndProof
import tech.pegasys.teku.phase1.integration.datastructures.LightClientVote
import tech.pegasys.teku.phase1.integration.datastructures.LightClientVoteData
import tech.pegasys.teku.phase1.integration.datastructures.PendingAttestation
import tech.pegasys.teku.phase1.integration.datastructures.ProposerSlashing
import tech.pegasys.teku.phase1.integration.datastructures.ShardBlock
import tech.pegasys.teku.phase1.integration.datastructures.ShardBlockHeader
import tech.pegasys.teku.phase1.integration.datastructures.ShardLatestMessage
import tech.pegasys.teku.phase1.integration.datastructures.ShardState
...
import tech.pegasys.teku.phase1.integration.datastructures.SignedCustodySlashing
import tech.pegasys.teku.phase1.integration.datastructures.SignedShardBlock
import tech.pegasys.teku.phase1.integration.datastructures.SignedVoluntaryExit
import tech.pegasys.teku.phase1.integration.datastructures.SigningData
import tech.pegasys.teku.phase1.integration.datastructures.Store
import tech.pegasys.teku.phase1.integration.datastructures.Validator
...
import tech.pegasys.teku.phase1.onotole.pylib.PyDict
import tech.pegasys.teku.phase1.onotole.pylib.PyList
import tech.pegasys.teku.phase1.onotole.pylib.Tuple2
import tech.pegasys.teku.phase1.onotole.pylib.all
...
import tech.pegasys.teku.phase1.onotole.pylib.copy
import tech.pegasys.teku.phase1.onotole.pylib.count
import tech.pegasys.teku.phase1.onotole.pylib.enumerate
import tech.pegasys.teku.phase1.onotole.pylib.filter
import tech.pegasys.teku.phase1.onotole.pylib.from_bytes
import tech.pegasys.teku.phase1.onotole.pylib.get
import tech.pegasys.teku.phase1.onotole.pylib.index
import tech.pegasys.teku.phase1.onotole.pylib.intersection
import tech.pegasys.teku.phase1.onotole.pylib.items
import tech.pegasys.teku.phase1.onotole.pylib.join
import tech.pegasys.teku.phase1.onotole.pylib.keys
import tech.pegasys.teku.phase1.onotole.pylib.len
import tech.pegasys.teku.phase1.onotole.pylib.list
import tech.pegasys.teku.phase1.onotole.pylib.map
import tech.pegasys.teku.phase1.onotole.pylib.max
...
import tech.pegasys.teku.phase1.onotole.pylib.shl
import tech.pegasys.teku.phase1.onotole.pylib.shr
import tech.pegasys.teku.phase1.onotole.pylib.slice
import tech.pegasys.teku.phase1.onotole.pylib.sorted
import tech.pegasys.teku.phase1.onotole.pylib.sum
import tech.pegasys.teku.phase1.onotole.pylib.times
import tech.pegasys.teku.phase1.onotole.pylib.toPyDict
import tech.pegasys.teku.phase1.onotole.pylib.toPyList
import tech.pegasys.teku.phase1.onotole.pylib.to_bytes
import tech.pegasys.teku.phase1.onotole.pylib.updateSlice
import tech.pegasys.teku.phase1.onotole.pylib.zip
...
import tech.pegasys.teku.phase1.onotole.ssz.uint32
import tech.pegasys.teku.phase1.onotole.ssz.uint64
import tech.pegasys.teku.phase1.onotole.ssz.uint8
import tech.pegasys.teku.phase1.onotole.ssz.uint_to_bytes
...
open class Phase1Spec(internal val bls: BLS) {
  ...
  fun compute_shuffled_index(index: uint64, index_count: uint64, seed: Bytes32): uint64 {
    ...
    for (current_round in range(SHUFFLE_ROUND_COUNT)) {
      ...
      val byte = source[((position % 256uL) / 8uL)].toUByte()
  ...
  fun compute_proposer_index(state: BeaconState, indices: Sequence<ValidatorIndex>, seed: Bytes32): ValidatorIndex {
    ...
    while (true) {
      ...
      if (((effective_balance * MAX_RANDOM_BYTE) >= (MAX_EFFECTIVE_BALANCE * random_byte.toUByte()))) {
  ...
  /*
      ...
      */
  open fun compute_committee(indices: Sequence<ValidatorIndex>, seed: Bytes32, index: uint64, count: uint64): Sequence<ValidatorIndex> {
  ...
  /*
      ...
      */
  open fun get_active_validator_indices(state: BeaconState, epoch: Epoch): Sequence<ValidatorIndex> {
  ...
  /*
      ...
      */
  open fun get_beacon_committee(state: BeaconState, slot: Slot, index: CommitteeIndex): Sequence<ValidatorIndex> {
  ...
  /*
      ...
      */
  open fun get_beacon_proposer_index(state: BeaconState): ValidatorIndex {
  ...
  /*
      ...
      */
  open fun get_total_active_balance(state: BeaconState): Gwei {
  ...
  fun initiate_validator_exit(state: BeaconState, index: ValidatorIndex): Unit {
    ...
    state.validators[index] = state.validators[index].updated {
      it.exit_epoch = exit_queue_epoch
      it.withdrawable_epoch = Epoch((validator.exit_epoch + MIN_VALIDATOR_WITHDRAWABILITY_DELAY))
    }
  }
  ...
  fun slash_validator(state: BeaconState, slashed_index: ValidatorIndex, whistleblower_index: ValidatorIndex? = null): Unit {
    ...
    val validator = state.validators[slashed_index].updated {
      it.slashed = true
      it.withdrawable_epoch = max(it.withdrawable_epoch, Epoch((epoch + EPOCHS_PER_SLASHINGS_VECTOR)))
    }
    state.validators[slashed_index] = validator
  ...
  fun initialize_beacon_state_from_eth1(eth1_block_hash: Bytes32, eth1_timestamp: uint64, deposits: Sequence<Deposit>): BeaconState {
    ...
    val state = BeaconState(
        ...
        randao_mixes = SSZVector(BasicViewTypes.BYTES32_TYPE, PyList(eth1_block_hash) * EPOCHS_PER_HISTORICAL_VECTOR))
    val leaves = list(map({ deposit -> deposit.data }, deposits))
    for ((index, deposit) in enumerate(deposits)) {
      val deposit_data_list = SSZList(DepositData.TYPE, 1uL shl DEPOSIT_CONTRACT_TREE_DEPTH, leaves.slice(0uL, (index.toULong() + 1uL)))
  ...
  open fun get_base_reward(state: BeaconState, index: ValidatorIndex): Gwei {
  ...
  /*
      ...
      */
  open fun get_inclusion_delay_deltas(state: BeaconState): Pair<Sequence<Gwei>, Sequence<Gwei>> {
  ...
  fun process_custody_key_reveal(state: BeaconState, reveal: CustodyKeyReveal): Unit {
    ...
    if (is_exited && is_exit_period_reveal) {
      revealer = revealer.copy(all_custody_secrets_revealed_epoch = get_current_epoch(state))
    }
    revealer = revealer.copy(next_custody_secret_to_reveal = revealer.next_custody_secret_to_reveal + 1uL)
    state.validators[reveal.revealer_index] = revealer
    val proposer_index = get_beacon_proposer_index(state)
  ...
  fun apply_shard_transition(state: BeaconState, shard: Shard, transition: ShardTransition): Unit {
    ...
    state.shard_states[shard] = transition.shard_states[(len(transition.shard_states) - 1uL)]
      .copy(slot = compute_previous_slot(state.slot))
  }
  ...
  fun process_crosslinks(state: BeaconState, shard_transitions: Sequence<ShardTransition>, attestations: Sequence<Attestation>): Unit {
    ...
    for (committee_index in map({ index -> CommitteeIndex(index) }, range(committee_count))) {
      ...
      if ((winning_root != Root())) {
        for ((index, pending_attestation) in enumerate(state.current_epoch_attestations)) {
          if (is_winning_attestation(state, pending_attestation, committee_index, winning_root)) {
            state.current_epoch_attestations[index] = pending_attestation.copy(crosslink_success = true)
          }
        }
      }
    }
  ...
  fun verify_shard_block_message(beacon_parent_state: BeaconState, shard_parent_state: ShardState, block: ShardBlock): pybool {
    ...
    var beacon_parent_block_header = beacon_parent_state.latest_block_header
  ...
  fun shard_state_transition(shard_state: ShardState, signed_block: SignedShardBlock, beacon_parent_state: BeaconState, validate_result: pybool = true): ShardState {
    ...
    return process_shard_block(shard_state, signed_block.message)
  }

  /*
      Update ``shard_state`` with shard ``block``.
      */
  fun process_shard_block(input_state: ShardState, block: ShardBlock): ShardState {
    return input_state.updated { shard_state ->
  ...
  fun is_valid_fraud_proof(beacon_state: BeaconState, attestation: Attestation, offset_index: uint64, transition: ShardTransition, block: ShardBlock, subkey: BLSPubkey, beacon_parent_block: BeaconBlock): pybool {
    ...
    shard_state = process_shard_block(shard_state, block)
  ...
  fun tech.pegasys.teku.datastructures.blocks.BeaconBlockHeader.toPhase1() = BeaconBlockHeader(slot = this.slot.toUInt64(), proposer_index = this.proposer_index.toUInt64(), parent_root = this.parent_root, state_root = this.state_root, body_root = this.body_root)
  ...
  fun upgrade_to_phase1(pre: tech.pegasys.teku.datastructures.state.BeaconState): BeaconState {
    ...
    var post = BeaconState(
        genesis_time = pre.genesis_time.toUInt64(),
        slot = pre.slot.toUInt64(),
        fork = Fork(
            previous_version = pre.fork.current_version,
            current_version = PHASE_1_FORK_VERSION,
            epoch = epoch),
        latest_block_header = pre.latest_block_header.toPhase1(),
        block_roots = SSZVector(BasicViewTypes.BYTES32_TYPE, pre.block_roots.asList()),
        state_roots = SSZVector(BasicViewTypes.BYTES32_TYPE, pre.state_roots.asList()),
        historical_roots = SSZList(BasicViewTypes.BYTES32_TYPE, HISTORICAL_ROOTS_LIMIT, pre.historical_roots.asList()),
        ...
        validators = SSZList(Validator.TYPE, VALIDATOR_REGISTRY_LIMIT, enumerate(pre.validators.asList()).map { (i, phase0_validator) ->
          Validator(
              ...
              effective_balance = phase0_validator.effective_balance.toUInt64(),
              slashed = phase0_validator.isSlashed,
              activation_eligibility_epoch = phase0_validator.activation_eligibility_epoch.toUInt64(),
              activation_epoch = phase0_validator.activation_eligibility_epoch.toUInt64(),
              exit_epoch = phase0_validator.exit_epoch.toUInt64(),
              withdrawable_epoch = phase0_validator.withdrawable_epoch.toUInt64(),
        ...
        balances = SSZList(BasicViewTypes.UINT64_TYPE, VALIDATOR_REGISTRY_LIMIT, pre.balances.asList().map { it.toUInt64() }),
        randao_mixes = SSZVector(BasicViewTypes.BYTES32_TYPE, pre.randao_mixes.asList()),
        slashings = SSZVector(BasicViewTypes.UINT64_TYPE, pre.slashings.asList().map { it.toUInt64() }),
        previous_epoch_attestations = SSZList(PendingAttestation.TYPE, MAX_ATTESTATIONS * SLOTS_PER_EPOCH),
        current_epoch_attestations = SSZList(PendingAttestation.TYPE, MAX_ATTESTATIONS * SLOTS_PER_EPOCH),
  ...
  fun get_shard_transition_fields(beacon_state: BeaconState, shard: Shard, shard_blocks: Sequence<SignedShardBlock>): Triple<Sequence<uint64>, Sequence<Root>, Sequence<ShardState>> {
    ...
    for (slot in offset_slots) {
      ...
      shard_state = process_shard_block(shard_state, shard_block.message)
  ...
  fun get_shard_transition(beacon_state: BeaconState, shard: Shard, shard_blocks: Sequence<SignedShardBlock>): ShardTransition {
    ...
    return ShardTransition(start_slot = offset_slots[0uL], shard_block_lengths = SSZList(BasicViewTypes.UINT64_TYPE, MAX_SHARD_BLOCKS_PER_ATTESTATION, shard_block_lengths), shard_data_roots = SSZList(BasicViewTypes.BYTES32_TYPE, MAX_SHARD_BLOCKS_PER_ATTESTATION, shard_data_roots), shard_states = SSZList(ShardState.TYPE, MAX_SHARD_BLOCKS_PER_ATTESTATION, shard_states), proposer_signature_aggregate = proposer_signature_aggregate)
  }
